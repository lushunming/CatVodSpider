...
public class Bili extends Spider {
    ...
    private void setCookie() {
        ...
        if (StringUtils.isEmpty(cookie)) cookie = Path.read(getCache());
        if (StringUtils.isEmpty(cookie)) cookie = COOKIE;
    }
...
//        setAudio();
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        for (Page page : detail.getPages()) episode.add(page.getPart() + "$" + aid + "+" + page.getCid() + "+" + StringUtils.join(acceptQuality, ":") + "+" + StringUtils.join(acceptDesc,":"));
        flag.put("B站", StringUtils.join(episode, "#"));
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            episode.add(object.get("title").getAsString() + "$" + object.get("aid").getAsInt() + "+" + object.get("cid").getAsInt() + "+" + StringUtils.join(acceptQuality, ":") + "+" + StringUtils.join(acceptDesc, ":"));
        }
        flag.put("相关", StringUtils.join(episode,"#"));

        vod.setVodPlayFrom(StringUtils.join(flag.keySet(), "$$$"));
        vod.setVodPlayUrl(StringUtils.join(flag.values(), "$$$"));
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) throws Exception {
        ...
        for (int i = 0; i < acceptDesc.length; i++) {
            ...
            url.add(Proxy.getProxyUrl() + "?do=bili" + "&aid=" + aid + "&cid=" + cid + "&qn=" + acceptQuality[i] + "&type=mpd");
        }
